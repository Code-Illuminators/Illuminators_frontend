...
        const creatureType = fullMarker.type === "default" ? "others" : fullMarker.type;    

        if (!window.confirm(`Are you sure you want to delete this sighting for ${creatureType} at ${fullMarker.location}?`)) {
            return;
        }
...
        fetch(`http://localhost:8000/api/posts/${creatureType}/${backendId}/delete/`, {
            method: 'DELETE',
            headers: {
                "Authorization": `Bearer ${accessToken}`,
            },
...
        const creatureTypeRoute = selectedCreature === "other" ? "others" : selectedCreature;
        
        const popUpText = locationMark;
...
    }
    
    const handlePromotion = () => { navigate("/promotion"); };
    const handleOngoingVotes = () => { navigate("/ongoing-votes"); };

    return (
        <>
            <nav className="navbar fixed-top glass-nav" data-bs-theme="dark">
                <div className="container-fluid">
                    <a className="navbar-brand logo-text" href="/">Code Illuminators</a>
...
                        <button 
                            className="btn btn-outline-light vote-btn-width" 
                            onClick={handleOngoingVotes}>Ongoing Votes
                        </button>
                    </div>
...
                        </button>
                    </div>
                    <div className="offcanvas offcanvas-end offcanvas-glass" id="offcanvasNavbar" tabIndex="-1" aria-labelledby="offcanvasNavbarLabel" style={{ "--bs-offcanvas-width": "340px" }}>
                        <div className="offcanvas-header">
                            <h5 className="offcanvas-title" id="offcanvasNavbarLabel">Code Illuminators</h5>
...
                                        onClick={() => !isSimpleUser && setSelectedCreature("bigfoot")}
                                        disabled={isSimpleUser}>Big Foot</button>
                                </li>
                                <li className="nav-item">
                                    <button 
                                        className={`nav-link d-block ${selectedCreature === "ufo" ? "active" : ""}`} 
                                        onClick={() => !isSimpleUser && setSelectedCreature("ufo")}
                                        disabled={isSimpleUser} >UFO</button>
                                </li>
                                <li className="nav-item">
                                    <button 
                                        className={`nav-link d-block ${selectedCreature === "ghosts" ? "active" : ""}`} 
                                        onClick={() => !isSimpleUser && setSelectedCreature("ghosts")}
                                        disabled={isSimpleUser}>Ghost</button>
                                </li>
                                <li className="nav-item">
                                    <input 
                                        type="text" 
...
                                    <label htmlFor="locationMark" className="form-label">Location</label>
                                    <input 
                                        type="text" 
                                        id="locationMark" 
...
                                </li>
                                <li className="nav-item mt-2">
                                    <label htmlFor="markerPhoto" className="form-label">Photo (required)</label>
                                    <input 
                                        type="file" 
                                        id="markerPhoto" 
                                        className="form-control" 
...
                            </button>
                            {blockMayday && (
                                <button className="compromised-btn" onClick={handleMayday}>WE ARE COMPROMISED</button>
                            )}
                        </div>
                    </div>
                </div>
...
                                        style={{maxWidth: '100px', display: 'block', margin: '5px 0'}} 
                                    />
                                )}
                                <DeleteButton 
                                    marker={marker} 
...
                
                {tempMarker && (
                    <Marker position={tempMarker.position} icon={icons[selectedCreature === "other" ? "default" : selectedCreature] || icons.default}>
